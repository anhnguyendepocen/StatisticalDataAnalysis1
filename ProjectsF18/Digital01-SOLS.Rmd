...
block   <- 3
x       <- 0:block
p.x     <- dbinom(x, block, 1 - p_bit)
redbars <- ifelse(x > 1, "red", "gray")

tibble(x = x, prob = p.x)

ggplot(NULL, aes(x = x, y = p.x)) + 
  geom_bar(stat = "identity", width = 1, fill = redbars) +
  labs(title = "pmf of X = number of bits correct out of 3",
       x = "x = number of bits correct",
       y = "probability of x")

p_block <- pbinom(1, block, 1 - p_bit, lower.tail = FALSE)
print(p_block)
```
...
```{r}
n_bits <- 10^9
EX2    <- n_bits * (1 - p_block)
SX2    <- sqrt(n_bits * p_block * (1 - p_block))
print(EX2)
print(SX2)
...
print.data.frame(data.frame(namelength = 2:15, nameprob = round(p_letter^(2:15), 4)),
                 row.names = FALSE)
```
...
```{r}
n_letters <- 1444
EX3       <- n_letters * (1 - p_letter)
SX3    <- sqrt(n_letters * p_letter * (1 - p_letter))
print(EX3)
print(SX3)
```
