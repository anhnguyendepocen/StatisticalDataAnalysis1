...
simdata <- pull(data, maze_time)  # all data points put in 1 pool
total_n <- length(simdata)        # overall number of data points
group_n <- summ$n[2]              # sample size of the Drug group
...
for (i in 1:N_sims) {
  ...
  # this is the same calculation used earlier to find obs_diff
  # index is the sampleIDs chosen and -index is IDs NOT chosen
  # so we have the Drug group mean minus the Control group mean
