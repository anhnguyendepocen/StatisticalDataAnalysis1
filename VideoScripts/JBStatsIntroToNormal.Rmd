...
<details>
```{r}
normalplot_1sigma <- ggplot(NULL) + 
  stat_function(fun = dnorm, args = list(0, 1), geom = "area", fill = "lightgray", xlim = c(-1, 1)) +
  stat_function(fun = dnorm, args = list(0, 1)) +
  geom_label(x = 0, y = 0.2, aes(label = "≈ 68%"), label.padding = unit(0.5, "lines")) +
  scale_x_continuous(limits = c(-3, 3),
                     breaks = seq(from = -3, to = 3, by = 1), 
                     labels = c("μ - 3σ", "μ - 2σ", "μ - 1σ", "μ", "μ + 1σ", "μ + 2σ", "μ + 3σ")) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
...
<details>
```{r}
normalplot_2sigma <- ggplot(NULL) + 
  stat_function(fun = dnorm, args = list(0, 1), geom = "area", fill = "lightgray", xlim = c(-2, 2)) +
  stat_function(fun = dnorm, args = list(0, 1)) +
  geom_label(x = 0, y = 0.2, aes(label = "≈ 95%"), label.padding = unit(0.5, "lines")) +
  scale_x_continuous(limits = c(-3, 3),
                     breaks = seq(from = -3, to = 3, by = 1), 
                     labels = c("μ - 3σ", "μ - 2σ", "μ - 1σ", "μ", "μ + 1σ", "μ + 2σ", "μ + 3σ")) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
...
<details>
```{r}
normalplot_3sigma <- ggplot(NULL) + 
  stat_function(fun = dnorm, args = list(0, 1), geom = "area", fill = "lightgray", xlim = c(-3, 3)) +
  stat_function(fun = dnorm, args = list(0, 1)) +
  geom_label(x = 0, y = 0.2, aes(label = "≈ 99.7%"), label.padding = unit(0.5, "lines")) +
  scale_x_continuous(limits = c(-3, 3),
                     breaks = seq(from = -3, to = 3, by = 1), 
                     labels = c("μ - 3σ", "μ - 2σ", "μ - 1σ", "μ", "μ + 1σ", "μ + 2σ", "μ + 3σ")) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
...
<details>
```{r}
normalplot_2curves <- ggplot(NULL) + 
  stat_function(fun = dnorm, args = list(0, 1), aes(color = "σ1")) +
  stat_function(fun = dnorm, args = list(0, 2), aes(color = "σ2 = 2σ1")) +
  scale_x_continuous(limits = c(-6, 6), breaks = NULL) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
  labs(title = "Two Normal Distributions", x = "x", y = "f(x)") +
...
The code below will allow you to plot a wide variety of normal distributions, within the limits of `R`. Just set the mean (mu) and standard deviation (sigma), and adjust the x-axis decimal display to be appropriate to the scale of the parameters.
...
ggplot(NULL) + 
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
  labs(title = sprintf("X ~ N(%g, %g)", mu, sigma^2), x = "x", y = "f(x)") +
  theme_linedraw()
```
...
ggplot(NULL) + 
  stat_function(fun = dnorm, args = list(mu, sigma), 
                geom = "area", fill = shade, xlim = c(lower, upper)) +
  stat_function(fun = dnorm, args = list(mu, sigma)) +
  scale_x_continuous(limits = c(LB, UB),
                     breaks = seq(from = LB, to = UB, by = sigma), 
                     labels = number_format(accuracy = decimals)) +
...
ggplot(NULL) + 
  stat_function(fun = dnorm, args = list(mu, sigma), 
                geom = "area", fill = shade, xlim = c(-10 * sigma, lower)) +
  stat_function(fun = dnorm, args = list(mu, sigma), 
                geom = "area", fill = shade, xlim = c(upper, 10 * sigma)) +
  stat_function(fun = dnorm, args = list(mu, sigma)) +
  scale_x_continuous(limits = c(LB, UB),
                     breaks = seq(from = LB, to = UB, by = sigma), 
                     labels = number_format(accuracy = decimals)) +
