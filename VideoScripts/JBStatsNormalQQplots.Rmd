...
```{r}
plotdata1 <- tibble(sample = c(3.89, 4.75, 4.75, 5.20, 5.78, 5.80, 6.33, 7.21, 7.90),
...
The continuous uniform distribution has a flatter peak and thinner tails than the normal distribution.
...
```{r}
ggplot() +
  stat_function(fun = dunif, args = list(-3, 3), color = "blue") +
  stat_function(fun = dnorm, args = list(0, 1), color = "orange") +
...
The Student's *t* distribution has thicker tails than the normal distribution when the parameter is small. However, the *t* converges to the standard normal distribution as the parameter approaches $\infty$.
...
ggplot(tibble(x = rnorm(n, 0, 1)), aes(sample = x)) +
  geom_qq() +
  geom_qq_line() +
  theme_classic()
```
...
norm_samples <- tibble(sample_values = rnorm(9 * n, 0, 1),
