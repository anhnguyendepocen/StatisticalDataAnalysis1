...
This code replaces the simulation loop above.

```{r simsamplestidy}
sim_means <- replicate(n = 1000,
          expr = {
            pop_ages %>% 
              slice_sample(n = 3) %>% 
...
Plotting the simulation results is identical.
...
```{r simstatstidy}
tibble(sim_means) %>% 
summarize(mean_sim_means = mean(sim_means),
          sd_sim_means   = sd(sim_means))
```
