...
We will be using the `dplyr` and `ggplot2` packages for our EDA. These tools, `readr`, and more are part of the `tidyverse`.
...
Other functions for exploring dataset structure are given below. Use them on the `Spruce` dataset. What information does each one give you?
...
You can find instructor-created data files on our [class data files](https://stat-jet-asu.github.io/Datasets/DatasetList.html) page. The packages `datasets`, `HistData`, `mosaicData`, `MASS`, and others contain additional datasets. You can check for data in any package using `data(package="packagename")`. Results will pop up in a tab in RStudio's Source pane.
