...
In addition to calculating probabilities, we can simulate random normal values in R using `rnorm (n, mu, sigma)`.

_n_ = the number of random values we want to generate

$\mu$ = the mean of the distribution
    
$\sigma$ = the standard deviation of the distribution

Default parameter settings in `rnorm()` are `mu = 0` and `sigma = 1`, so if we want a sample from the standard normal distribution, we only need to enter sample size, `rnorm(n)`.
