...
Note that the `R` function uses standard deviation rather than variance as the parameter. 
...
$P(X \leq x)$ = $F(x)$ = `pnorm(q, mean, sd)`

$P(X > x)$ = $1-F(x)$ = `pnorm` with `lower.tail = FALSE`

$q^{th}$ percentile (approx) = $F^{-1}(x)$ = `qnorm(p, mean, sd)` 
