...
```{r}
glimpse(galtonhts)
xbarF     <- mean(galtonhts$Father)
stdvF     <- sd(galtonhts$Father)
```
...
print.data.frame(tibble(data = quantile(galtonhts$Father, deciles), 
                        theory = round(qnorm(deciles, xbarF, stdvF),2)))
```
...
```{r, echo = FALSE}
plotdata <- tibble(data = quantile(galtonhts$Father, deciles), 
...
skewK <- tibble(Statistic = c("Skewness", "Kurtosis", "Excess K"),
                Theory = c(0, 3, 0),
                Data   = round(c(skewness(galtonhts$Father), 
                                 kurtosis(galtonhts$Father), 
                                 kurtosis(galtonhts$Father) - 3), 2))
...
Assess the other height measurements in the Galton dataset for normality. For the child data, perform the assessment with and without grouping by gender. Was Quetelet right to think human height is normally distributed?
