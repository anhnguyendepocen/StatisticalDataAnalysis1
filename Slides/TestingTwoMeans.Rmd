...
How many different unique recombinations of the data are possible?
...
What are all possible unique combinations? Choose data for "brand".

```{r}
combinations <- combn(12, 6)
combinations[ , 1:10]
```

Take a closer look at one combination from among the possibilities.

```{r}
alldata <- data.frame(index = 1:12, hours = battery$hours)
print.data.frame(alldata, row.names = FALSE)

index   <- combinations[ ,1]
brand   <- battery$hours[index]
