...
What is the probability that either one or both of two particular events happen? In other words, for one randomly chosen case, only $A$ _or_ $B$ must be true, but both may be true in some cases.

*  $P(A \text{ or } B) = P(A) + P(B) - P(A \text{ and } B)$

*  $P(A \text{ or } B) = P(A) + P(B)$, if $A$ and $B$ are mutually exclusive

If A and B are mutually exclusive, $P(A \text{ and } B) = 0$ &rarr; impossible!
...
* $A$ and $B$ are independent $\iff P(A \text{ and } B) = P(A)P(B)$
