...
A __dataset__ is a collection of information for all observations and variables in a study (e.g., survey or experiment). A common way to arrange data is to put variables in columns and observations in rows. This is what we call `tidy` data. 
...
Many different types of data files can be read into R. We will be using primarily CVS files read from the web using their URL. Our [Class Data Files](https://stat-jet-asu.github.io/Datasets/DatasetList.html) page contains links to many data files provided by the instructor and the textbook.
...
The `read.csv()` function converts a CVS file into a __data frame__.
