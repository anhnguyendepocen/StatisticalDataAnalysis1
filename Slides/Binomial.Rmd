...
To generate `s` random binomial values, use `rbinom (s, n, p)`.
...
Assuming we conduct a test with an open deck as described... 

* What are the parameters of the binomial model?
* Calculate the pmf and CDF for X = number correct.
* Display the pmf and CDF in a data frame or tibble.
* Plot the pmf. Color the "significant" X values (10 or more).
* Plot the pmf. Color the "significant" X values (more than 10).
* Plot the CDF. Color the "significant" X values (10 or more).
* Plot the CDF. Color the "significant" X values (more than 10).
