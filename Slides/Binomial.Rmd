...
$P(X \leq x)$ = $F(x)$ = `pbinom(q, n, p)`

$P(X > x)$ = $1-F(x)$ = use `lower.tail = FALSE` (carefully)

$q^{th}$ percentile (approx) = $F^{-1}(x)$ = `qbinom(p, n, p)` 
...
Be careful, since the function notation in `R` could be confused with the Binomial parameters!
