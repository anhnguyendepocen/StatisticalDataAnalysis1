...
For a given number of successes $x$ in a series of $n$ independent Bernoulli trials with $p$ chance of success for each trial, the pmf finds the probability of a single sequence with $x$ successes and multiplies it by the total possible number of such sequences.
...
The most likely outcome is two heads. The second most likely is three heads. No surprise, given the coin's bias.
...
For a large value of $x$ this can get quite tedious. Unfortunately there is no shortcut, but we have technology!
...
These quantities are also known as *cumulative probabilities*.
...
ggplot(stacked) +
  ...
  scale_y_continuous(limits = c(0, 1.05),
                     breaks = seq(0, 1, 0.05),
                     expand = c(0,0)) +
  labs(title = "CDF of X ~ Bin(3, 0.7) Segmented by pdf Values",
       x = "number of successes",
       y = "probability") +
  theme_linedraw()
```
...
The expected (mean) number of successes and the variance of the number of successes are found as follows. You can see the proof [**HERE**](https://www.probabilisticworld.com/binomial-distribution-mean-variance-formulas-proof/). In general: 
